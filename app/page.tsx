...
function Friends(){return <div className="screen-content"><Header kicker="THE GRINCH'S ACCOMPLICES" title="Our Friends" text="Businesses, organizations and community partners who have supported — or been gotten by — the Siouxville Grinch."/><div className="friends-grid">{FRIENDS.map((friend)=>friend.url?<a className="friend-card" href={friend.url} target="_blank" rel="noreferrer" key={friend.name}><div className="friend-logo"><FriendLogo friend={friend}/></div><b>{friend.name}</b><small>Visit website ↗</small></a>:<div className="friend-card no-link" key={friend.name}><div className="friend-logo"><FriendLogo friend={friend}/></div><b>{friend.name}</b><small>Community friend</small></div>)}</div></div>}
