...
type View = Tab|"about"|"media"|"reviews"|"sponsor"|"sponsor-form"|"sponsor-info"|"sponsors-list"|"bored"|"pricing"|"gallery"|"coloring"|"friends";
...
function Sponsor({go}:{go:(v:View)=>void}){return <div className="screen-content"><Header kicker="BECOME A SPONSOR" title="Partner with the Grinch" text="The Siouxville Grinch is proud to partner with local businesses, creators, and community supporters. Sponsorships help bring more holiday magic to Sioux Falls and beyond. If you'd like to support our events, appearances, or community projects, we'd love to work with you."/><div className="sponsor-hub"><button className="card sponsor-choice" onClick={()=>go("sponsor-form")}><HandHeart/><div><h3>Become a Sponsor</h3><p>Choose a sponsorship tier, activation, dates and send your request directly to the Grinch.</p></div><ChevronRight/></button><button className="card sponsor-choice" onClick={()=>go("sponsors-list")}><Users/><div><h3>Our Sponsors</h3><p>Meet the businesses and community supporters helping cause holiday mischief.</p></div><ChevronRight/></button><button className="card sponsor-choice" onClick={()=>go("sponsor-info")}><Info/><div><h3>Sponsorship Info</h3><p>View the official Siouxville Grinch sponsorship information PDF.</p></div><ChevronRight/></button></div></div>}

function SponsorForm(){
 const [status,setStatus]=useState<"idle"|"sending"|"sent"|"error">("idle"); const [error,setError]=useState("");
 async function submit(e:FormEvent<HTMLFormElement>){e.preventDefault();setStatus("sending");setError("");const form=e.currentTarget;const d=new FormData(form);const payload={business:String(d.get("business")||""),contact:String(d.get("contact")||""),email:String(d.get("email")||""),phone:String(d.get("phone")||""),tier:String(d.get("tier")||""),addon:String(d.get("addon")||""),activation:String(d.get("activation")||""),date1:String(d.get("date1")||""),date2:String(d.get("date2")||""),startTime:String(d.get("startTime")||""),endTime:String(d.get("endTime")||""),notes:String(d.get("notes")||"")};try{const res=await fetch("/api/sponsorship",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)});const result=await res.json().catch(()=>({}));if(!res.ok)throw new Error(result?.error||"The Grinch's sponsorship mailroom had a problem.");setStatus("sent");form.reset()}catch(err){setStatus("error");setError(err instanceof Error?err.message:"Unable to send sponsorship request.")}}
 return <div className="screen-content"><Header kicker="SPONSORSHIP FORM" title="Become a Sponsor" text="Complete the form below and your sponsorship request will be delivered directly to the Siouxville Grinch."/><form className="form-card card" onSubmit={submit}><label>Business / Organization Name *<input name="business" required disabled={status==="sending"}/></label><label>Primary Contact Name *<input name="contact" required disabled={status==="sending"}/></label><label>Contact Email *<input name="email" type="email" required disabled={status==="sending"}/></label><label>Contact Phone *<input name="phone" type="tel" required disabled={status==="sending"}/></label><label>Preferred Sponsorship Tier *<select name="tier" required defaultValue="" disabled={status==="sending"}><option value="" disabled>Select a tier</option><option>$250 Mischief Maker</option><option>$500 Holiday Helper</option><option>$1,000 Presenting Sponsor</option></select></label><label>Optional Add-On<select name="addon" defaultValue="No add-on" disabled={status==="sending"}><option>No add-on</option><option>$150 Mask Sponsor</option><option>$1,500 Suit Sponsor</option><option>$20 - $150 Parade Sponsor</option><option>$20 - $100 Print Sponsor</option><option>$50 - $200 Sponsor a Visit</option></select></label><label>Preferred Activation *<select name="activation" required defaultValue="" disabled={status==="sending"}><option value="" disabled>Select activation</option><option>Pop-up visit</option><option>Open house</option><option>Charity tie-in</option><option>Custom</option></select></label><div className="two"><label>Desired Date 1 *<input name="date1" type="date" required disabled={status==="sending"}/></label><label>Desired Date 2 *<input name="date2" type="date" required disabled={status==="sending"}/></label></div><div className="two"><label>Start Time *<input name="startTime" type="time" required disabled={status==="sending"}/></label><label>End Time *<input name="endTime" type="time" required disabled={status==="sending"}/></label></div><div className="logo-note"><Mail/><span>Please email your logo separately to <b>{EMAIL}</b>.</span></div><label>Notes * <small>(up to 2,500 characters)</small><textarea name="notes" required maxLength={2500} rows={8} disabled={status==="sending"} placeholder="Tell the Grinch about your sponsorship plans..."/></label>{status==="sent"&&<div className="form-notice success"><b>Sponsorship request sent!</b><span>The Grinch's lair has received your request. You can reply separately with your logo.</span></div>}{status==="error"&&<div className="form-notice error"><b>That didn't send.</b><span>{error}</span></div>}<button className="primary wide" type="submit" disabled={status==="sending"}><Send/>{status==="sending"?"Sending...":"Submit Sponsorship Form"}</button></form></div>}
function SponsorInfo(){return <div className="screen-content"><Header kicker="SPONSORSHIP" title="Sponsorship Information" text="Review the official sponsorship information prepared for Siouxville Grinch partners."/><section className="card sponsor-info-card"><Info/><h3>Official Sponsorship Information</h3><p>The sponsorship PDF includes the current program information and can be opened in your device's PDF viewer.</p><a className="primary wide top-gap" href={`${SITE}/sponsorship-info.pdf`} target="_blank" rel="noreferrer">Open Sponsorship PDF</a></section></div>}
function SponsorsList(){return <div className="screen-content"><Header kicker="OUR SPONSORS" title="The Sponsor Lair" text="Businesses and community supporters helping the Siouxville Grinch spread holiday mischief will appear here."/><section className="card empty-sponsors"><Users/><h3>Our sponsor lair is being prepared...</h3><p>Check back soon as our first sponsors join the mischief.</p></section></div>}
